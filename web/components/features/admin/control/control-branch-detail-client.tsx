...
export function ControlBranchDetailClient({
  ...
  const tabs: Array<{ key: BranchDetailTab; label: string }> = [
    ...
    ...(canViewFinance ? [{ key: 'finance' as const, label: t('tabs.finance') }] : []),
  ...
  const hoursByEmployeeData = useMemo(
    () =>
      teamPayroll.employees
        ...
        .sort(
          (left, right) =>
            right.approvedHours + right.pendingHours - (left.approvedHours + left.pendingHours)
        )
        .slice(0, 8),
    [teamPayroll.employees]
  );
  ...
  const handleSaveSetup = async (nextSettings: BranchSetupState): Promise<boolean> => {
    setSavingSetup(true);
    try {
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 rounded-[28px] border border-slate-200 bg-white p-4">
        <div className="flex items-start gap-3">
          <Button
            variant="ghost"
            size="sm"
            className="mt-0.5 h-8 shrink-0 rounded-lg px-2"
            onClick={() => router.push(`/${appLocale}/control/restaurants`)}
            aria-label={t('backToRestaurants')}
          >
            <ArrowLeft className="h-4 w-4" />
            <span className="hidden sm:ml-1 sm:inline">{t('backToRestaurants')}</span>
          </Button>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-x-4 gap-y-3 rounded-[24px] border border-slate-200 bg-white p-4 sm:grid-cols-4">
            <div className="space-y-0.5">
              <p className="text-xs text-muted-foreground">{t('stats.todayRevenue')}</p>
              <p className="text-xl font-semibold tabular-nums text-emerald-600">
                {formatCurrency(overview.today_revenue, financeCurrency, appLocale)}
              </p>
            ...
            <div className="space-y-0.5">
              <p className="text-xs text-muted-foreground">{t('stats.openOrders')}</p>
              <p
                className={cn(
              ...
              <p className="text-xs text-muted-foreground">{t('stats.inKitchen')}</p>
            </div>
            <div className="space-y-0.5">
              <p className="text-xs text-muted-foreground">{t('stats.monthSpending')}</p>
              <p className="text-xl font-semibold tabular-nums text-slate-900">
                {formatCurrency(overview.month.spending_total, financeCurrency, appLocale)}
              </p>
              <p className="text-xs text-muted-foreground">{t('stats.monthSpendingHint')}</p>
            </div>
            <div className="space-y-0.5">
              <p className="text-xs text-muted-foreground">
              ...
              {overview.month.has_closed_snapshot ? (
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-sm font-medium text-emerald-600">{t('status.closed')}</span>
                </div>
              ) : (
                <div className="flex items-center gap-1.5">
                  <XCircle className="h-5 w-5 text-amber-500" />
                  <span className="text-sm font-medium text-amber-600">{t('status.open')}</span>
                </div>
              )}
              {overview.month.has_closed_snapshot ? (
                <p className="text-xs text-muted-foreground">
                  {formatCurrency(overview.month.revenue_total, financeCurrency, appLocale)}
                </p>
              ) : null}
            </div>
          </div>
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    <TableHead className="px-4 py-3">{t('info.tableLabel')}</TableHead>
                    <TableHead className="px-4 py-3">{t('info.tableValue')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  <TableRow>
                    <TableCell className="px-4 py-3 text-sm font-medium">{t('info.name')}</TableCell>
                    <TableCell className="px-4 py-3 text-sm text-slate-600">
                      {initialBranch.name}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="px-4 py-3 text-sm font-medium">{t('info.code')}</TableCell>
                    <TableCell className="px-4 py-3 text-sm text-slate-600">
                      {initialBranch.branch_code ?? initialBranch.subdomain}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="px-4 py-3 text-sm font-medium">{t('info.address')}</TableCell>
                    <TableCell className="px-4 py-3 text-sm text-slate-600">
                      {initialBranch.address ?? t('common.notSet')}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="px-4 py-3 text-sm font-medium">{t('info.contact')}</TableCell>
                    <TableCell className="px-4 py-3 text-sm text-slate-600">
                      {initialBranch.phone || initialBranch.email
                        ? [initialBranch.phone, initialBranch.email].filter(Boolean).join(' · ')
                        : t('common.notSet')}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                  ...
                  <TableRow>
                    <TableCell className="px-4 py-3 text-sm font-medium">{t('info.bestSeller')}</TableCell>
                    <TableCell className="px-4 py-3 text-sm text-slate-600">
                      {topSellingItem
            ...
            <div className="overflow-hidden rounded-[24px] border border-slate-200 bg-white">
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    <TableHead className="px-4 py-3">{t('snapshot.tableLabel')}</TableHead>
                    <TableHead className="px-4 py-3">{t('snapshot.tableValue')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  <TableRow>
                    <TableCell className="px-4 py-3 text-sm font-medium">{t('snapshot.period')}</TableCell>
                    <TableCell className="px-4 py-3 text-sm text-slate-600">{monthLabel}</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="px-4 py-3 text-sm font-medium">{t('snapshot.status')}</TableCell>
                    <TableCell className="px-4 py-3 text-sm text-slate-600">
                      {overview.month.has_closed_snapshot ? t('status.closed') : t('status.open')}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="px-4 py-3 text-sm font-medium">{t('snapshot.revenue')}</TableCell>
                    <TableCell className="px-4 py-3 text-sm text-slate-600">
                      {formatCurrency(overview.month.revenue_total, financeCurrency, appLocale)}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    ...
                    <TableCell className="px-4 py-3 text-sm text-slate-600">
                      {formatCurrency(overview.month.gross_profit, financeCurrency, appLocale)}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="px-4 py-3 text-sm font-medium">
                    ...
                    <TableCell className="px-4 py-3 text-sm text-slate-600">
                      {t('stats.openOrdersCount', { count: overview.open_orders_count })}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="px-4 py-3 text-sm font-medium">
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            ...
            >
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={revenueTrendData} margin={{ top: 4, right: 4, left: -12, bottom: 0 }}>
                  <defs>
                    <linearGradient id="branchRevenue14d" x1="0" y1="0" x2="0" y2="1">
          ...
          <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
            <div id="branch-best-sellers" className="rounded-[24px] border border-slate-200 bg-white p-4">
              <div className="mb-4">
                <p className="text-sm font-medium text-slate-900">{t('bestSellers.title')}</p>
                <p className="mt-0.5 text-xs text-slate-500">{t('bestSellers.subtitle')}</p>
              </div>
              {hasTopItems ? (
                <div className="space-y-3">
                  {overview.top_items_30d.map((item, index) => {
                    const maxQuantity = overview.top_items_30d[0]?.quantity ?? 1;
                    const width = Math.round((item.quantity / maxQuantity) * 100);

                    return (
                      <div key={`${item.item_name}-${index}`} className="space-y-1">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <p className="text-xs tabular-nums text-slate-500">
                            {t('bestSellers.itemMeta', {
                              ...
                              revenue: formatCurrency(item.revenue, financeCurrency, appLocale),
                            })}
                          </p>
                        </div>
            ...
            <div className="rounded-[24px] border border-slate-200 bg-white p-4">
              <div className="mb-4">
                <p className="text-sm font-medium text-slate-900">{t('hoursSnapshot.title')}</p>
                <p className="mt-0.5 text-xs text-slate-500">{t('hoursSnapshot.subtitle')}</p>
              </div>
              <div className="space-y-3">
                <div className="rounded-2xl border bg-slate-50 px-4 py-3">
                  <p className="text-xs text-slate-500">{t('hoursSnapshot.approvedHours')}</p>
                  <p className="mt-1 text-lg font-semibold text-emerald-700">
                    {formatHours(teamPayroll.totals.approvedHours)}
                  </p>
                </div>
                <div className="rounded-2xl border bg-slate-50 px-4 py-3">
                  <p className="text-xs text-slate-500">{t('hoursSnapshot.pendingReview')}</p>
                  <p className="mt-1 text-lg font-semibold text-amber-600">
                    {formatHours(teamPayroll.totals.pendingHours)}
                  </p>
                </div>
                <div className="rounded-2xl border bg-slate-50 px-4 py-3">
                  <p className="text-xs text-slate-500">{t('hoursSnapshot.reviewLabel')}</p>
                  <p className="mt-1 text-sm text-slate-700">{t('hoursSnapshot.reviewBody')}</p>
                </div>
                <Button
                  variant="outline"
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>{t('team.employeeCount', { count: employees.length })}</span>
            </div>
            <Button
              size="sm"
            ...
            <div className="space-y-4 rounded-[24px] border border-slate-200 bg-white p-4">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setAddForm((current) => ({ ...current, job_title: value }))
                    }
                  >
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className="gap-1.5 rounded-lg"
                >
                  {addingEmployee ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : null}
          ...
          {employees.length === 0 ? (
            <div className="rounded-[24px] border border-dashed border-slate-200 py-14 text-center">
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t('team.emptyDescription')}</p>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="rounded-[24px] border border-slate-200 bg-white p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <h3 className="text-base font-semibold">{t('team.rosterTitle')}</h3>
                    <p className="mt-1 text-sm text-muted-foreground">{t('team.rosterDescription')}</p>
                  </div>
                  <Badge variant="secondary" className="rounded-full">
