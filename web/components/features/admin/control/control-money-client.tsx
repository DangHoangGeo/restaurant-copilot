...
function closeProgress(
  ...
  return periodType === "quarter"
    ? `${branchesWithSnapshot}/3`
    : branchesWithSnapshot > 0
      ? "1/1"
      : "0/1";
}
...
function StatCell({
  ...
  return (
    <div className="rounded-[22px] border border-[#C8773E]/14 bg-[#FFFDF8] px-4 py-3">
      <p className="text-xs text-[#7A5D45]">{label}</p>
      <p
        className={cn(
          "mt-1 truncate text-lg font-semibold tabular-nums text-[#24170F]",
          tone === "green" && "text-[#55763F]",
          tone === "amber" && "text-amber-700",
          tone === "blue" && "text-[#A95F2F]",
...
function Section({
  ...
  return (
    <section className="overflow-hidden rounded-[26px] border border-[#C8773E]/16 bg-[#FFFDF8] shadow-sm shadow-[#8A4E24]/5">
      <div className="flex flex-col gap-3 border-b border-[#EAD7BD] bg-[#FFF7E9] px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-sm font-semibold text-[#24170F]">{title}</h2>
          {description ? (
            <p className="mt-1 text-xs text-[#7A5D45]">{description}</p>
          ) : null}
        </div>
        {action}
      </div>
...
export function ControlMoneyClient({
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5">
      <section className="rounded-[30px] bg-[#14100B] p-5 text-[#FFF7E9] shadow-xl shadow-[#8A4E24]/18">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-[#E9A35E]">
            ...
            <div>
              <h1 className="text-2xl font-semibold leading-tight text-[#FFF7E9]">
                {t("company.pageTitle")}
              </h1>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap lg:max-w-[520px] lg:justify-end">
            <div className="flex items-center gap-1 rounded-xl border border-[#F1DCC4]/14 bg-[#FFF7E9]/8 p-1">
              <Button
                type="button"
                variant="ghost"
                size="sm"
                className={cn(
                  "rounded-lg text-[#DBC7AD] hover:bg-[#FFF7E9]/12 hover:text-[#FFF7E9]",
                  periodType === "month" &&
                    "bg-[#C8773E] text-white hover:bg-[#A95F2F]",
                )}
            ...
            <div className="flex items-center gap-1 rounded-xl border border-[#F1DCC4]/14 bg-[#FFF7E9]/8 p-1">
              <Button
                ...
                variant="ghost"
                size="icon"
                className="h-8 w-8 rounded-lg text-[#DBC7AD] hover:bg-[#FFF7E9]/12 hover:text-[#FFF7E9]"
                onClick={() => navigatePeriod(-1)}
                aria-label={t("company.controls.previousPeriod")}
              >
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <div className="min-w-[110px] px-2 text-center text-sm font-medium text-[#FFF7E9]">
                {report.periodLabel}
              </div>
              <Button
                type="button"
            ...
            <Select
              value={selectedScope}
              onValueChange={(value) =>
                updateFilters({ restaurantId: value === "all" ? null : value })
              }
            >
              <SelectTrigger className="h-10 min-w-[180px] rounded-xl border-[#F1DCC4]/14 bg-[#FFF7E9]/8 text-[#FFF7E9]">
                <SelectValue placeholder={t("company.controls.branchFilter")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">
                  {t("company.controls.allBranches")}
                </SelectItem>
                {branches.map((branch) => (
            ...
            {canManageExpenses ? (
              <OrganizationExpenseDialog currency={currency} />
            ) : null}
            ...
            {canExport ? (
              <Button
                ...
                variant="outline"
                size="sm"
                className="h-10 rounded-xl border-[#F1DCC4]/14 bg-[#FFF7E9]/8 text-[#FFF7E9] hover:bg-[#FFF7E9]/12"
          ...
          <>
            ...
            <StatCell
              ...
              value={formatCurrency(
                report.estimated_sales_tax_total,
                currency,
                locale,
              )}
      ...
      <Tabs defaultValue="summary" className="space-y-4">
        <TabsList
          className={`grid w-full rounded-[18px] border border-[#C8773E]/14 bg-[#FFFDF8] p-1 ${tabValues.length === 4 ? "grid-cols-4" : tabValues.length === 3 ? "grid-cols-3" : "grid-cols-2"}`}
        >
          <TabsTrigger value="summary">{t("company.tabs.summary")}</TabsTrigger>
          <TabsTrigger value="branches">
            {t("company.tabs.branches")}
          </TabsTrigger>
          {canViewIncome ? (
            <TabsTrigger value="tax">{t("company.tabs.tax")}</TabsTrigger>
          ) : null}
          {canViewSpending ? (
            <TabsTrigger value="shared-expenses">
              {t("company.tabs.sharedExpenses")}
            </TabsTrigger>
          ) : null}
        </TabsList>
              ...
              {chartData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={260}>
                  ...
                  >
                    ...
                    <Tooltip
                      content={
                        <FinanceTooltip currency={currency} locale={locale} />
                      }
                    />
                    {canViewIncome ? (
                      <Bar
              ...
              <Table>
                <TableBody>
                  {canViewIncome ? (
                    <>
                      <TableRow>
                        <TableCell className="px-0 py-3 text-slate-500">
                          {t("grossSales")}
                        </TableCell>
                      ...
                      <TableRow>
                        <TableCell className="px-0 py-3 text-slate-500">
                          {t("company.summary.netSalesInclusive")}
                        </TableCell>
                        <TableCell className="px-0 py-3 text-right font-medium text-slate-900">
                          {formatCurrency(
                  ...
                  {canViewSpending ? (
                    <>
                      ...
                      <TableRow>
                        <TableCell className="px-0 py-3 text-slate-500">
                          {t("company.summary.branchExpenses")}
                        </TableCell>
                        <TableCell className="px-0 py-3 text-right font-medium text-slate-900">
                          {formatCurrency(
                            report.branch_expense_total,
                            currency,
                            locale,
                          )}
                        </TableCell>
                      ...
                      <TableRow>
                        <TableCell className="px-0 py-3 text-slate-500">
                          {t("company.summary.companyExpenses")}
                        </TableCell>
                        <TableCell className="px-0 py-3 text-right font-medium text-slate-900">
                          {formatCurrency(
                            report.company_expense_total,
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="text-right">
                    {t("company.branches.close")}
                  </TableHead>
                  <TableHead className="text-right">
                    {t("company.branches.action")}
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                  ...
                  <TableRow key={branch.restaurant_id}>
                    ...
                    {canViewIncome ? (
                      <>
                        ...
                        <TableCell className="text-right tabular-nums text-amber-600">
                          {formatCurrency(
                            branch.estimated_sales_tax_total,
                            currency,
                            locale,
                          )}
                        </TableCell>
                    ...
                    <TableCell className="text-right">
                      ...
                      >
                        <Link
                          href={`/${locale}/control/restaurants/${branch.restaurant_id}?tab=finance&year=${year}&month=${month}`}
                        >
              ...
              >
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl bg-slate-50 p-4">
                    <p className="text-xs text-slate-500">
                      {t("company.sharedExpenses.total")}
                    </p>
                  ...
                  <div className="rounded-2xl bg-slate-50 p-4">
                    <p className="text-xs text-slate-500">
                      {t("company.sharedExpenses.entryCount")}
                    </p>
                    <p className="mt-1 text-lg font-semibold text-slate-900">
                ...
                <div className="mt-4 space-y-3">
                  {expenseCategories.length === 0 ? (
                    ...
                    expenseCategories.map((row) => {
                      ...
                      const width =
                        max > 0 ? Math.max((row.amount / max) * 100, 8) : 0;
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent">
                      ...
                      <TableHead>
                        {t("company.sharedExpenses.category")}
                      </TableHead>
                      <TableHead>
                        {t("company.sharedExpenses.descriptionColumn")}
                      </TableHead>
                      <TableHead>
                        {t("company.sharedExpenses.vendor")}
                      </TableHead>
                      <TableHead>
                        {t("company.sharedExpenses.recordedBy")}
                      </TableHead>
                      <TableHead className="text-right">
                        {t("company.sharedExpenses.amount")}
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
