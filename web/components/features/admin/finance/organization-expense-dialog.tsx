...
export function OrganizationExpenseDialog({
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch('/api/v1/owner/organization/finance/expenses', {
        method: 'POST',
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-lg rounded-[28px]">
          <DialogHeader>
            <DialogTitle>{tFinance('company.sharedExpenses.dialogTitle')}</DialogTitle>
            <DialogDescription>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder={tFinance('company.sharedExpenses.descriptionPlaceholder')}
              />
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{tFinance('company.sharedExpenses.vendor')}</Label>
                <Input
                  ...
                  className="h-10 rounded-xl"
                  placeholder={tFinance('company.sharedExpenses.vendorPlaceholder')}
                />
              </div>
            </div>
            ...
            <DialogFooter className="gap-2 sm:justify-between">
              <p className="text-xs text-muted-foreground">
                {tFinance('company.sharedExpenses.auditHint')}
              </p>
