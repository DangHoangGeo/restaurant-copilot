...
export function AdminBottomNav({ branchPermissions }: AdminBottomNavProps) {
  ...
  const canAccessItem = (item: NavItem | ActionItem) =>
    !item.requiredPermission || branchPermissions[item.requiredPermission];

  const allMoreItems: ActionItem[] = [
    {
      icon: ClipboardList,
  ...
  const moreItems = allMoreItems.filter(canAccessItem);
    ...
    <>
      ...
      >
        <ul className="flex justify-around">
          {visibleNavItems.map(({ icon: Icon, labelKey, href, exact }) => {
