    ...
    try {
      ...
      if (initialData?.id) {
        ...
        response = await fetch(
          `/api/v1/owner/categories/${initialData.id}?branchId=${encodeURIComponent(restaurantId)}`,
          {
            method: "PUT",
            headers: {
        ...
        // Creating
        response = await fetch(
          `/api/v1/owner/categories?branchId=${encodeURIComponent(restaurantId)}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      toast(
        initialData?.id ? t("update_success_desc") : t("create_success_desc"),
      );
      router.push(buildBranchPath(locale, restaurantId, "menu"));
      router.refresh();
    } catch (error) {
