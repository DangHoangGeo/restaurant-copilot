...
describe("resolveScopedBranchRouteAccess", () => {
  ...
  it("prefers organization scope over the legacy branch shortcut", async () => {
    ...
    await expect(
      resolveScopedBranchRouteAccess("branch-a"),
    ).resolves.toBeNull();
  });
  ...
  it("allows organization members to access an authorized branch", async () => {
    ...
    await expect(
      resolveScopedBranchRouteAccess("branch-b"),
    ).resolves.toMatchObject({
      branchId: "branch-b",
    });
  });

  it("denies organization members from branch pages without the required permission", async () => {
    mockedGetUserFromRequest.mockResolvedValue({
      userId: "user-1",
      ...
      organization: {
        organizationId: "org-1",
        role: "founder_finance",
        accessibleRestaurantIds: ["branch-a"],
        permissionOverrides: {},
      },
    });
    ...
    await expect(
      resolveScopedBranchRouteAccess("branch-a", "employees"),
    ).resolves.toBeNull();
  });

  it("allows branch page access when an explicit permission override grants it", async () => {
    mockedGetUserFromRequest.mockResolvedValue({
      userId: "user-1",
      email: "finance@example.com",
      restaurantId: "branch-a",
      ...
      organization: {
        ...
        role: "founder_finance",
        accessibleRestaurantIds: ["branch-a"],
        permissionOverrides: { employees: true },
      },
    });
    ...
    mockedResolveOrgContext.mockResolvedValue({
      ...
      accessibleRestaurantIds: ["branch-a"],
      permissionOverrides: new Map([["employees", true]]),
    });
