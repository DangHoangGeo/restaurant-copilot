...
export async function closeSnapshot(
  ...
  if (!insertError) {
    return inserted as MonthlyFinanceSnapshot;
  }

  if (insertError.code !== "23505") {
    throw new Error(`closeSnapshot insert: ${insertError.message}`);
  }
  ...
  const { data: updated, error: updateError } = await supabaseAdmin
    ...
    .eq("snapshot_status", "draft")
    .select("*")
    .maybeSingle();

  if (updateError) {
    throw new Error(`closeSnapshot update: ${updateError.message}`);
  }
  ...
  return updated as MonthlyFinanceSnapshot;
}
...
export async function computeLaborHours(
  ...
  const approved_labor_hours = rows.reduce(
...
export async function computePurchasing(
  ...
  if (ordersResult.error)
    throw new Error(`computePurchasing orders: ${ordersResult.error.message}`);
  if (expensesResult.error)
    throw new Error(
      `computePurchasing expenses: ${expensesResult.error.message}`,
    );
 ...
 */
export async function getBranchIdsForOrg(
...
export async function computeDiscounts(
  ...
  const discount_total = rows.reduce(
    (sum, row) => sum + (row.discount_amount ?? 0),
    0,
  );
...
export function buildLiveSummary(params: {
  ...
  return {
    ...
    gross_profit_estimate:
      params.revenue_total - params.discount_total - params.combined_cost_total,
  };
}
