...
import { expenseCategoryValues, purchaseCategoryValues } from '@/lib/server/purchasing/schemas';
...
export async function getBranchFinanceDetail(params: {
  ...
  const [salesResult, expensesResult, purchasesResult] = await Promise.all([
    supabaseAdmin
      .from('orders')
      .select('id, total_amount, created_at, tables(name), order_items(quantity)')
  ...
  if (salesResult.error) throw new Error(`getBranchFinanceDetail sales: ${salesResult.error.message}`);
  if (expensesResult.error) throw new Error(`getBranchFinanceDetail expenses: ${expensesResult.error.message}`);
  if (purchasesResult.error) throw new Error(`getBranchFinanceDetail purchases: ${purchasesResult.error.message}`);

  const creatorIds = [
    ...(expensesResult.data ?? []).map((row) => row.created_by).filter(Boolean),
    ...(purchasesResult.data ?? []).map((row) => row.created_by).filter(Boolean),
  ] as string[];
  const creatorMap = await loadCreatorMap(Array.from(new Set(creatorIds)));

  const recentSales: BranchFinanceRecentSale[] = (salesResult.data ?? []).map((row) => {
  ...
  const purchaseCategorySpend = purchaseCategoryValues.map((category) => ({
    ...
    amount: (purchasesResult.data ?? [])
      .filter((row) => row.category === category)
      .reduce((sum, row) => sum + ((row.total_amount as number | null) ?? 0), 0),
  }));

  const recentExpenses: BranchFinanceRecentExpense[] = (expensesResult.data ?? []).map((row) => ({
    id: row.id as string,
    ...
    created_by_name:
      ((row.created_by as string | null) && creatorMap.get(row.created_by as string)) ?? null,
  }));

  const recentPurchases: BranchFinanceRecentPurchase[] = (purchasesResult.data ?? []).map((row) => ({
    id: row.id as string,
    category: row.category as string,
