...
import { useForm, Controller } from 'react-hook-form' // Added Controller for Select in bulk form
import { zodResolver } from '@hookform/resolvers/zod'
import { z } from 'zod'
import { PlusCircle, SquarePen, QrCode, FileDown, Loader2, Layers } from 'lucide-react' // Added Layers
...
const getBulkAddTableSchema = (t: ReturnType<typeof useTranslations<'AdminTables.validation'>>) => z.object({
  ...
  capacity: z.number({ required_error: t('capacity_required') }).min(1, t('capacity_min', { min: 1 })),
  status: z.enum(['available', 'occupied', 'reserved'], { required_error: t('status_required') }).default('available'),
  isOutdoor: z.boolean().default(false),
  isAccessible: z.boolean().default(false),
});
    ...
    <div>
      ...
      <Dialog open={isTableModalOpen} onOpenChange={setIsTableModalOpen}>
        <DialogContent>
          ...
          <Form {...{ control: controlSingle, handleSubmit: handleSubmitSingle, reset: resetSingle, formState: { errors: errorsSingle }, setValue: setValueSingle, watch: watchSingle }}>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const res = await fetch(`/api/v1/tables/${selectedTableForQr.id}`, {
                      method: 'PATCH',
                      headers: { 'Content-Type': 'application/json' },
