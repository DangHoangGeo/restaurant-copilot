...
import { List, Calendar, UserPlus, SquarePen, CalendarCheck2 } from 'lucide-react'

const MOCK_EMPLOYEES_BASE = [
  { id: 'emp1', name: 'Alice Smith', role: 'manager', email: 'alice@example.com', shifts: { Mon: '09:00-17:00', Tue: '09:00-17:00', Wed: null, Thu: '09:00-17:00', Fri: '09:00-17:00', Sat: null, Sun: null } },
  { id: 'emp2', name: 'Bob Johnson', role: 'chef', email: 'bob@example.com', shifts: { Mon: '10:00-18:00', Tue: '10:00-18:00', Wed: '10:00-18:00', Thu: null, Fri: '10:00-22:00', Sat: '12:00-22:00', Sun: null } },
  { id: 'emp3', name: 'Carol Williams', role: 'server', email: 'carol@example.com', shifts: { Mon: null, Tue: '17:00-22:00', Wed: '17:00-22:00', Thu: '17:00-22:00', Fri: '17:00-23:00', Sat: '17:00-23:00', Sun: '12:00-20:00' } }
]
...
export function EmployeesClientContent() {
  ...
  const employees = MOCK_EMPLOYEES_BASE
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <h2 className="text-2xl font-semibold text-slate-800 dark:text-slate-100">{t('AdminNav.admin_employees_title')}</h2>
            ...
            <table className="w-full min-w-[1000px] text-sm border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-700">
                  <th className="sticky left-0 z-10 bg-slate-50 dark:bg-slate-700 p-2 border dark:border-slate-600 text-left">{t('AdminEmployees.employee_name')}</th>
                  {daysOfWeek.map(day => <th key={day} className="p-2 border dark:border-slate-600 text-center">{t(`Common.weekdays.${day.toLowerCase()}`)}</th>)}
                </tr>
              </thead>
              <tbody>
                {employees.map(emp => (
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingEmployee?.id ? t('AdminEmployees.edit_employee') : t('AdminEmployees.add_employee')}</DialogTitle>
          </DialogHeader>
          {editingEmployee && (
            <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-3">
              <Input name="name" value={editingEmployee.name} onChange={e => setEditingEmployee({ ...editingEmployee, name: e.target.value })} placeholder={t('AdminEmployees.form.full_name')} />
              <Input name="email" type="email" value={editingEmployee.email} onChange={e => setEditingEmployee({ ...editingEmployee, email: e.target.value })} placeholder={t('AdminEmployees.form.email_lookup_placeholder')} />
