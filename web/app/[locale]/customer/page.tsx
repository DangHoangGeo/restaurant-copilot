...
export default async function CustomerHomePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  setRequestLocale(locale)
  const host = (await headers()).get('host') || ''
  const subdomain = getSubdomainFromHost(host)
  const restaurantSettings = subdomain ? await getRestaurantSettingsFromSubdomain(subdomain) : null
  return <CustomerClientContent restaurantSettings={restaurantSettings ?? { name: 'Restaurant', logoUrl: null }} />
}
