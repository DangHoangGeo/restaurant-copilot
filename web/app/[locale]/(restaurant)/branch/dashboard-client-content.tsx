...
import {
  RecentOrdersTable,
  RecentOrder,
} from "@/components/features/admin/dashboard/RecentOrdersTable";
import {
  PopularItemsList,
  PopularItem,
} from "@/components/features/admin/dashboard/PopularItemsList";
import {
  LowStockAlerts,
  LowStockItem,
} from "@/components/features/admin/dashboard/LowStockAlerts";
...
const SalesOverTimeChart = dynamic(
  () =>
    import("@/components/features/admin/dashboard/SalesOverTimeChart").then(
      (mod) => mod.SalesOverTimeChart,
    ),
  {
    ssr: false,
...
export function DashboardClientContent() {
  ...
  const { needsOnboarding, isLoading: contextLoading } =
        ...
        <Card className="text-center">
          <CardHeader className="pb-6">
            ...
            <p className="text-muted-foreground mt-2">
              Let&apos;s get your restaurant set up in just a few minutes. Our
              AI will help you create a stunning profile and homepage.
            </p>
          </CardHeader>
          <CardContent className="pb-8">
            <div className="space-y-4 max-w-md mx-auto">
              ...
              <Link href={buildBranchPath(locale, branchId, "onboarding")}>
  ...
  const topSellerName =
    metrics?.topSellerToday?.name ?? t("cards.top_seller_unavailable");
