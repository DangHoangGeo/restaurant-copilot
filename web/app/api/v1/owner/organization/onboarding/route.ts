...
export async function POST(request: NextRequest) {
  ...
  if (ctx!.organization.approval_status !== "approved") {
    return forbidden(
      "Organization must be approved before onboarding can be completed",
    );
  }
  ...
  try {
    ...
    if (!result.success) {
      return NextResponse.json(
        { error: result.error ?? "Failed to save onboarding" },
        { status: 500 },
      );
    }
    ...
    if (restaurantError) {
      console.error(
        "Organization onboarding restaurant save failed:",
        restaurantError,
      );
      return NextResponse.json(
        { error: "Failed to save branch setup" },
        { status: 500 },
      );
    }
    ...
    if (branch.gallery_images) {
      ...
      if (deleteGalleryError) {
        console.error(
          "Failed to reset onboarding gallery images:",
          deleteGalleryError,
        );
        return NextResponse.json(
          { error: "Failed to save branch gallery" },
          { status: 500 },
        );
      }
    }
    ...
    if (branch.gallery_images && branch.gallery_images.length > 0) {
      ...
      if (galleryError) {
        console.error(
          "Failed to save onboarding gallery images:",
          galleryError,
        );
        return NextResponse.json(
          { error: "Failed to save branch gallery" },
    ...
    if (branch.signature_dishes) {
      ...
      if (deleteSignatureItemsError) {
        console.error(
          "Failed to reset signature dishes during onboarding:",
          deleteSignatureItemsError,
        );
        return NextResponse.json(
    ...
    if (branch.signature_dishes && branch.signature_dishes.length > 0) {
      ...
      if (!categoryId) {
        ...
        if (categoryError || !newCategory) {
          console.error(
            "Failed to create onboarding signature category:",
            categoryError,
          );
          return NextResponse.json(
            { error: "Failed to save starter signature dishes" },
      ...
      if (signatureError) {
        console.error(
          "Failed to save onboarding signature dishes:",
          signatureError,
        );
        return NextResponse.json(
          { error: "Failed to save starter signature dishes" },
    ...
    const { data: updatedRestaurant, error: updatedRestaurantError } =
      await supabaseAdmin
        .from("restaurants")
        .select("*")
        .eq("id", branch.id)
    ...
    if (updatedRestaurantError) {
      console.error(
        "Failed to fetch updated onboarding branch:",
