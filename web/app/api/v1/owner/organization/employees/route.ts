...
const createEmployeeSchema = z.object({
  ...
  job_title: z.enum(["manager", "chef", "server", "cashier", "part_time"]),
});
...
export async function GET() {
  ...
  const employees = await listOrganizationEmployees(
    ctx!.accessibleRestaurantIds,
  );
  ...
  try {
    ...
    if (inviteError) {
      ...
      if (
        ...
        if (!existingUser) {
          return NextResponse.json(
            { error: "User already registered but not found in system" },
            { status: 409 },
          );
        }
        ...
        if (existingEmp) {
          return NextResponse.json(
            { error: "This person is already an employee at this restaurant" },
            { status: 409 },
          );
        }
    ...
    if (!userId) {
      await logger.error(
        "org-employees-post",
        "Invite API returned missing user",
        {
          restaurant_id,
          email,
        },
      ...
      return NextResponse.json(
        { error: "Failed to create employee from invite response" },
        { status: 500 },
      );
    }
    ...
    const { error: userUpsertError } = await supabaseAdmin
      .from("users")
      .upsert(
        { id: userId, email, name, role: USER_ROLES.EMPLOYEE, restaurant_id },
        { onConflict: "id" },
      );
    ...
    if (empError) {
      ...
      return NextResponse.json(
        { error: "Invite sent but failed to create employee record" },
        { status: 500 },
      );
    }
