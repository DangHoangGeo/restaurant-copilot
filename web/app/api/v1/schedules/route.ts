...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (error) {
      return NextResponse.json(
        { message: "Error fetching schedules", details: error.message },
        { status: 500 },
      );
    }
    ...
    console.error("API Error in GET schedules:", error);
    return NextResponse.json(
      {
  ...
  try {
    ...
    if (newScheduleData.restaurant_id !== user.restaurantId) {
      return NextResponse.json(
        {
          error:
            "Forbidden: Mismatch in restaurant ID. Cannot create schedule for another restaurant.",
        },
        { status: 403 },
      );
    }
    ...
    if (employeeCheckError || !employeeCheck) {
      ...
      return NextResponse.json(
        {
          error:
            "Employee not found in your restaurant or error validating employee.",
        },
        { status: 404 },
      );
    }
    ...
    if (insertError) {
      ...
      if (insertError.code === "23505") {
        // Unique constraint violation (e.g., duplicate schedule)
        return NextResponse.json(
          {
            error:
              "This shift overlaps with an existing shift or violates a constraint.",
          },
          { status: 409 },
        );
      }
